...
- Run `npm run build`, `npm run lint`, and `npm run test:api` before submitting.
- Verify Firebase Authentication Email/Password is enabled.
- Verify the Firestore composite index for `ownerId + updatedAt` is deployed.
- Confirm `.env`, `server/.env`, and `server/secrets/` are ignored by git.
- Record a short walkthrough showing registration, quiz creation, publish/share, anonymous attempt, and scoring.
